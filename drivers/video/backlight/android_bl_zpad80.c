...
struct android_bl_data {
	...
	struct regulator *lcd_bklight_reg;
        ...
        #ifdef CONFIG_BOARD_ZPAD80
	struct regulator *lcd_vcc_reg_en;
...
static void android_bl_on(struct android_bl_data *dev) {
	...
	#ifdef CONFIG_BOARD_ZPAD80
	regulator_enable(dev->lcd_vcc_reg_en);
...
static void bk_e_suspend(struct early_suspend *h)
{
	...
	#ifdef CONFIG_BOARD_ZPAD80
	regulator_disable(dev->lcd_vcc_reg_en);
...
static int __devinit android_bl_probe(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_BOARD_ZPAD80
	dev->lcd_vcc_reg_en = regulator_get(NULL, "vlcd_en");

	if (IS_ERR(dev->lcd_vcc_reg_en)) {
		dev_err(&pdev->dev, "failed to get regulator vlcd_en\n");
		return PTR_ERR(dev->lcd_vcc_reg_en);
    	}
...
static int __devinit android_bl_remove(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_BOARD_ZPAD80
	regulator_put(dev->lcd_vcc_reg_en);
