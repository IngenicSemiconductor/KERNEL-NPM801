/*
 * linux/include/asm-mips/mach-jz4780/jz4780nemc.h
 *
 * JZ4780 NEMC register definition.
 *
...
#define NEMC_CS3	0x19000000 /* read-write area in static bank 3 */
#define NEMC_CS4	0x18000000 /* read-write area in static bank 4 */
#define NEMC_CS5	0x17000000 /* read-write area in static bank 5 */
#define NEMC_CS6	0x16000000 /* read-write area in static bank 6 */

#define NEMC_BANK0	0x1C000000 /* static bank 0 */
#define NEMC_CS(n)	(NEMC_BANK0 - (n) * 0x01000000)
...
#define NEMC_TGRR_TD		(1 << 16) /* Timer Done */
#define NEMC_TGRR_CWAW_BIT	0 /* Command Write Cycle to Address Write Cycle Time */
#define NEMC_TGRR_CWAW_MASK	(0xFF << NEMC_TGRR_CWAW_BIT)
/* Toggle NAND DQS Delay Control */
#define NEMC_TGDR_ERR		(1 << 29) /* DQS Delay Detect ERROR */
#define NEMC_TGDR_DONE		(1 << 28) /* Delay Detect Done */
...
#define __tnand_td_sync()	while (!(REG_NEMC_TGRR & NEMC_TGRR_TD))
...
#define __tnand_enable(n) \
do { \
        REG_NEMC_NFCSR = NEMC_NFCSR_TNFE(n) | NEMC_NFCSR_NFE(n); \
        __tnand_dphtd_sync(n); \
        REG_NEMC_NFCSR |= NEMC_NFCSR_NFCE(n) | NEMC_NFCSR_DAEC; \
} while (0)

#define __tnand_disable(n) \
do { \
...
#define __tnand_random_cd(delay) \
do { \
	REG_NEMC_TGRR = (delay) & NEMC_TGRR_CWAW_MASK; \
	__tnand_td_sync(); \
} while (0)
